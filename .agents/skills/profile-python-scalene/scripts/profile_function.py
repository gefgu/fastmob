...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-kind",
        choices=("brightkite-pandas", "brightkite-polars", "geolife-pandas", "none"),
        default="brightkite-pandas",
        help="Input builder for the target's first positional argument.",
    )
    parser.add_argument("--rows", type=positive_int, default=1000, help="Rows to pass from the selected dataset.")
    parser.add_argument("--repeat", type=positive_int, default=3, help="Number of profiled calls after one warmup call.")
    parser.add_argument("--kwargs-json", default="{}", help="JSON object passed as keyword arguments to the target.")
    parser.add_argument(
        "--data-path",
        type=Path,
        default=BRIGHTKITE_PATH,
        help="Brightkite data path for brightkite input kinds.",
    )
...
def build_input(args: argparse.Namespace) -> Any:
    ...
    if args.input_kind == "geolife-pandas":
        from tests.shared.geolife import load_geolife_pandas

        return load_geolife_pandas(mode="slice", rows=args.rows)
