---
icon: lucide/users-round
---

# Collective Measures

| API | Description |
| --- | --- |
| [`homes_per_location`](#fastmob.measures.collective.homes_per_location) | Return the number of users whose home is at each distinct location. |
| [`mean_square_displacement`](#fastmob.measures.collective.mean_square_displacement) | Return the mean square displacement (km^2) across all users. |
| [`random_location_entropy`](#fastmob.measures.collective.random_location_entropy) | Return the random entropy for each distinct location across all users. |
| [`uncorrelated_location_entropy`](#fastmob.measures.collective.uncorrelated_location_entropy) | Return the uncorrelated entropy for each distinct location across all users. |
| [`visits_per_location`](#fastmob.measures.collective.visits_per_location) | Return the total number of visits for each distinct global location ID. |
| [`visits_per_time_unit`](#fastmob.measures.collective.visits_per_time_unit) | Return the number of trajectory points per time unit across all users. |
| [`od_matrix`](#fastmob.measures.collective.od_matrix) | Compute an Origin-Destination matrix from a trips DataFrame. |
| [`od_metrics_per_area`](#fastmob.measures.collective.od_metrics_per_area) | Compute per-area mobility metrics from a long-format OD DataFrame. |
| [`mean_area_volume`](#fastmob.measures.collective.mean_area_volume) | Mean user volume per area and 10-minute time bin, averaged across days of the week. |
| [`build_stvd`](#fastmob.measures.collective.build_stvd) | Aggregate staypoints against a global Locations catalogue into an STVD frame. |

::: fastmob.measures.collective.homes_per_location
    options:
      show_source: false

---

::: fastmob.measures.collective.mean_square_displacement
    options:
      show_source: false

---

::: fastmob.measures.collective.random_location_entropy
    options:
      show_source: false

---

::: fastmob.measures.collective.uncorrelated_location_entropy
    options:
      show_source: false

---

::: fastmob.measures.collective.visits_per_location
    options:
      show_source: false

---

::: fastmob.measures.collective.visits_per_time_unit
    options:
      show_source: false

---

::: fastmob.measures.collective.od_matrix
    options:
      show_source: false

---

::: fastmob.measures.collective.od_metrics_per_area
    options:
      show_source: false

---

::: fastmob.measures.collective.mean_area_volume
    options:
      show_source: false

---

::: fastmob.measures.collective.build_stvd
    options:
      show_source: false
