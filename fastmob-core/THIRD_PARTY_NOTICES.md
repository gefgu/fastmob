...
The exact upstream commit/tag these were ported from has not been recorded;
pin one before publish.
...
code. The exact upstream commit/tag has not been recorded; pin one before
publish.
...
Software. The exact upstream commit/tag has not been recorded; pin one
before publish.
...
source rather than independently reimplemented. Remaining pre-publish
checklist items:

- Record the exact upstream commit/tag each module was ported from (MoveTK,
  PTRAIL, `hampel`, HuMobi entries above are missing this).
