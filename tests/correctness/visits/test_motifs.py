...
def test_digraph_self_isomorphic():
    ...
    from skmob2.measures.visits.motifs import _DiGraph, _is_isomorphic
...
def test_motif_library_has_17_graphs():
    ...
    from skmob2.measures.visits.motifs import get_motif_library
...
def test_all_motifs_have_correct_types():
    ...
    from skmob2.measures.visits.motifs import get_motif_library, _DiGraph
    library = get_motif_library()
...
# ---------------------------------------------------------------------------
# _motif_id / canonical ID format tests
# ---------------------------------------------------------------------------

def test_motif_id_single_node():
    """Single-node, no-edge graph → canonical ID 'm1:0'."""
    from skmob2.measures.visits.motifs import _DiGraph, _motif_id
    g = _DiGraph(n_nodes=1, edges=frozenset())
    assert _motif_id(g) == "m1:0"


def test_motif_id_simple_return():
    """Bidirectional 2-node graph → canonical ID 'm2:0110'."""
    from skmob2.measures.visits.motifs import _DiGraph, _motif_id
    g = _DiGraph(n_nodes=2, edges=frozenset({(0, 1), (1, 0)}))
    assert _motif_id(g) == "m2:0110"


def test_motif_id_isomorphic_graphs_agree():
    """Two isomorphic graphs produce the same canonical ID."""
    from skmob2.measures.visits.motifs import _DiGraph, _motif_id
    # Cycle 0→1→2→0 and its relabeling 1→2→0→1
    g1 = _DiGraph(n_nodes=3, edges=frozenset({(0, 1), (1, 2), (2, 0)}))
    g2 = _DiGraph(n_nodes=3, edges=frozenset({(1, 2), (2, 0), (0, 1)}))
    assert _motif_id(g1) == _motif_id(g2)
...
def test_dynamic_library_structure():
    ...
    from skmob2.measures.visits.motifs import create_dynamic_motif_library_object_from_static
...
def test_classify_motif_2_returns_known_id():
    """The simple return graph (2 nodes, 2 bidirectional edges) classifies to 'm2:0110'."""
    from skmob2.measures.visits.motifs import (
        _DiGraph,
    ...
    assert motif_id == "m2:0110"
    assert is_new is False


def test_classify_motif_1_returns_known_id():
    """Single-node graph classifies to 'm1:0'."""
    from skmob2.measures.visits.motifs import (
        _DiGraph,
        create_dynamic_motif_library_object_from_static,
        classify_or_add_motif_v2,
...
def test_load_missing_file_initializes(tmp_path):
    ...
    from skmob2.measures.visits.motifs import load_dynamic_motif_library
...
def test_build_motif_graph_returns_digraph():
    ...
    from skmob2.measures.visits.motifs import build_motif_graph, _DiGraph
...
def test_build_motif_graph_home_work_home_is_simple_return():
    ...
    from skmob2.measures.visits.motifs import build_motif_graph
    daily_df = _make_daily_df()
    g = build_motif_graph(daily_df, primary_home_node_id="home_loc_HOME")
    assert g.n_nodes == 2
