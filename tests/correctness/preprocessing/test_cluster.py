"""Correctness tests for fastmob.preprocessing.cluster."""
...
def test_cluster_noise_label_minus_one():
    ...
    labels = list(
        result["cluster"] if hasattr(result, "__getitem__") else [r["cluster"] for r in result.iter_rows(named=True)]
    )
    assert all(int(label) == -1 for label in labels)
...
def test_cluster_matches_skmob(comparison_skmob):
    ...
    our_result = cluster(
        our_stops, cluster_radius_km=0.1, datetime_col="datetime", lat_col="lat", lng_col="lng", uid_col="uid"
    )
