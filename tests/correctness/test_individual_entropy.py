"""Tests for trajectory entropy and predictability measures (Step 8)."""
import math
import pandas as pd
import numpy as np
import pytest
...
def test_kontoyiannis_entropy_constant_lower_than_varied():
    ...
    """
    from skmob2.measures.individual import _kontoyiannis_entropy
...
def test_fano_solve_zero_entropy_returns_one():
    ...
    from skmob2.measures.individual import _solve_max_predictability_with_fano
...
def test_fano_solve_predictability_in_range():
    ...
    for entropy in [0.0, 0.5, 1.0, 2.0, math.log2(n)]:
        ...
        assert 1.0 / n - 1e-6 <= p <= 1.0 + 1e-6, (
            f"Out of range for entropy={entropy}: predictability={p}"
        )
...
def test_fano_solve_monotone_decreasing_in_entropy():
    ...
    predictabilities = [
        _solve_max_predictability_with_fano(e, n) for e in entropies
    ]
...
def test_trajectory_entropy_result_shape():
    ...
    from skmob2.measures.individual import trajectory_entropy
    df = _entropy_visits()
...
def test_trajectory_predictability_result_columns():
    ...
    from skmob2.measures.individual import trajectory_predictability
    df = _entropy_visits()
    result = trajectory_predictability(df)
    expected_cols = {"agent_id", "real_entropy", "predictability",
                     "n_unique_locations", "n_steps"}
