"""Correctness tests for fastmob/measures/visits/location_frequency.py."""
...
def test_location_frequency_known_values(synthetic_tdf):
    ...
    from fastmob.measures.individual.location_frequency import location_frequency
...
def test_location_frequency_matches_skmob(comparison_skmob):
    ...
    fastmob_result = fastmob_lf(fastmob_input)
    ...
    fastmob_dict = _to_freq_dict(fastmob_result)
    ...
    for uid in common_uids:
        ...
        for loc in common_locs:
            assert abs(skmob_dict[uid][loc] - fastmob_dict[uid][loc]) < 1e-5, (
