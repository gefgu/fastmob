...
def test_build_profile_command_function_scope_uses_pid_placeholder(tmp_path):
    ...
    assert profile.output_path == tmp_path / "fastmob" / "radius_of_gyration.json.gz"
...
def test_runner_dry_run_writes_manifest(tmp_path):
    ...
    assert manifest[0]["output_path"].endswith("fastmob/radius_of_gyration.json.gz")
...
def test_runner_dry_run_both_uses_implementation_specific_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "select_workloads", lambda requested, implementation="fastmob": ["radius_of_gyration"])
    args = Namespace(
        rows=10_000,
        workload=["radius_of_gyration"],
        output_dir=str(tmp_path),
