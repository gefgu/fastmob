...
def test_prepare_jump_lengths_method_uses_fastmob_trajdataframe(monkeypatch):
...
def test_prepare_jump_lengths_function_uses_fastmob_trajdataframe(monkeypatch):
    monkeypatch.setattr(workloads, "load_brightkite", lambda rows, *, backend: _tiny_brightkite())
...
def test_non_jump_lengths_trajectory_workload_keeps_plain_dataframe(monkeypatch):
    monkeypatch.setattr(workloads, "load_brightkite", lambda rows, *, backend: _tiny_brightkite())
    workload = workloads.workload_registry("fastmob")["radius_of_gyration"]
