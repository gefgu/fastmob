...
use crate::utils::{arrow_u64_values, as_nullable_u64_array, u64_results_into_arrow};
...
pub fn interest_network(
    ...
    let user_codes = as_nullable_u64_array(user_codes, "user_codes")?;
    let location_ranks = as_nullable_u64_array(location_ranks, "location_ranks")?;
    ...
    let edges = py
        .detach(|| {
            interest_network_impl(
                ...
                arrow_u64_values(&location_ranks),
            )
        })
    ...
    Ok((
        Py::new(py, u64_results_into_arrow(location_a))?.into_any(),
        Py::new(py, u64_results_into_arrow(location_b))?.into_any(),
