...
use skmob2_core::measures::individual::recency_rank::{
    recency_rank_indexed_impl, recency_rank_indexed_values_impl, recency_rank_presorted_impl,
};
...
type RecencyRankValuesNumpy<'py> = (
    Bound<'py, PyArray1<f64>>,
    Bound<'py, PyArray1<f64>>,
...
type RecencyRankValuesArrow<'py> = (PyArray, PyArray, PyArray, Bound<'py, PyArray1<usize>>);
...
#[pyfunction]
pub fn recency_rank_values_indexed_numpy<'py>(
    py: Python<'py>,
    latitudes: PyReadonlyArray1<'py, f64>,
    longitudes: PyReadonlyArray1<'py, f64>,
    indices: PyReadonlyArray1<'py, usize>,
    ends: PyReadonlyArray1<'py, usize>,
) -> PyResult<RecencyRankValuesNumpy<'py>> {
...
#[pyfunction]
pub fn recency_rank_values_indexed_arrow<'py>(
    py: Python<'py>,
    latitudes: PyArray,
    longitudes: PyArray,
    indices: PyReadonlyArray1<'py, usize>,
    ends: PyReadonlyArray1<'py, usize>,
) -> PyResult<RecencyRankValuesArrow<'py>> {
    let latitudes = as_nullable_f64_array(latitudes, "latitudes")?;
    let longitudes = as_nullable_f64_array(longitudes, "longitudes")?;
    let valid_rows = arrow_valid_rows(&[&latitudes, &longitudes]);
    let (out_lats, out_lngs, out_ranks, out_user_indices) = recency_rank_indexed_values_impl(
...
#[pyfunction]
pub fn recency_rank_presorted_numpy<'py>(
    py: Python<'py>,
    latitudes: PyReadonlyArray1<'py, f64>,
    longitudes: PyReadonlyArray1<'py, f64>,
    ends: PyReadonlyArray1<'py, usize>,
...
#[pyfunction]
pub fn recency_rank_presorted_arrow<'py>(
    py: Python<'py>,
    latitudes: PyArray,
    longitudes: PyArray,
    ends: PyReadonlyArray1<'py, usize>,
