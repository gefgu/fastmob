...
from benchmarks import speed_visits_suite as suite
...
def test_catalogs_include_expected_entries():
    ...
    measure_names = {
        ...
        if entry["suite"] in {"individual", "collective"}
    }
    assert {"frequency_rank", "mean_square_displacement", "visits_per_time_unit"}.issubset(measure_names)
