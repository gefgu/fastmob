...
def test_output_path_matches_library_backend_and_timing_mode(tmp_path: Path):
    assert (
        ...
        == tmp_path / "fastmob_privacy_speed_pandas.json"
    )
    assert (
    ...
    assert (
        ...
        == tmp_path / "fastmob_privacy_speed_sorted_pandas.json"
    )
    assert (
...
def test_parse_args_expands_both_input_orders():
    args = suite.parse_args(["--library", "fastmob", "--input-order", "both"])
...
def test_benchmark_attack_warms_up_then_records_iterations(monkeypatch):
    ...
    result = suite.benchmark_attack(spec, "fastmob", lambda: object(), iterations=2, sleep_seconds=0.0)
