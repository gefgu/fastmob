...
def test_output_path_matches_library_and_profile(tmp_path: Path):
    assert suite.build_output_path(tmp_path, "fastmob") == tmp_path / "fastmob_models_speed.json"
...
def test_selected_specs_filters_metrics():
    args = suite.parse_args(["--library", "fastmob", "--metrics", "epr", "gravity_flows"])
...
def test_parse_args_accepts_sizes_as_location_alias():
    args = suite.parse_args(["--library", "fastmob", "--sizes", "7", "11"])
...
def test_write_json_serializes_payload(tmp_path: Path):
    output_path = suite.write_json({"metadata": {"library": "fastmob"}, "results": []}, tmp_path / "result.json")

    assert output_path == tmp_path / "result.json"
    assert json.loads(output_path.read_text(encoding="utf-8")) == {
        "metadata": {"library": "fastmob"},
        "results": [],
    }
...
def test_benchmark_model_records_import_errors():
    ...
    result = suite.benchmark_model(spec, "fastmob", object(), object(), iterations=1, sleep_seconds=0.0)
...
def test_build_call_passes_agent_and_location_dimensions(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "fastmob.models", fake_models)
...
def test_fastmob_smoke_with_fake_models(monkeypatch, tmp_path: Path):
    pd = pytest.importorskip("pandas")
