...
def test_output_path_matches_library_profile_and_synthetic_source(tmp_path: Path):
    assert (
        suite.build_output_path(tmp_path, "skmob2")
        == tmp_path / "skmob2_evaluation_speed_synthetic.json"
    )
    assert (
...
def test_parse_args_selects_library():
    assert suite.parse_args(["--library", "skmob2"]).library == "skmob2"
...
def test_skmob2_tiny_synthetic_smoke():
