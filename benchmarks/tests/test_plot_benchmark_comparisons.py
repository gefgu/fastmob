...
def test_generate_plots_models_location_only(tmp_path: Path, monkeypatch):
    ...
    optimized_json = tmp_path / "fastmob_models_speed.json"
    original_json.write_text(json.dumps(_location_model_payload("skmob", [12, 50])), encoding="utf-8")
    optimized_json.write_text(json.dumps(_location_model_payload("fastmob", [12, 50])), encoding="utf-8")
...
def test_generate_plots_groups_model_matrix_by_agents(tmp_path: Path, monkeypatch):
    ...
    optimized_json.write_text(json.dumps(_model_matrix_payload("fastmob")), encoding="utf-8")
...
def test_comparison_title_does_not_repeat_backend():
    assert plot.comparison_title("spatial", "polars") == "fastmob vs skmob"
    assert plot.comparison_title("visits", "pandas") == "fastmob vs skmob"
...
def _model_args(original_json: Path, optimized_json: Path, tmp_path: Path) -> argparse.Namespace:
    return argparse.Namespace(
        ...
        sizes=None,
        sort="speedup",
        large_json_fastmob=None,
        large_json_skmob=None,
        large_loc_json_fastmob=None,
