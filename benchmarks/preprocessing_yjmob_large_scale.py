...
Usage:
    ...
    python benchmarks/preprocessing_yjmob_large_scale.py --n-users 1000 10000 100000
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    for n_users in sorted(args.n_users):
        for backend in backends:
            print(f"Benchmarking latlng_to_h3: n_users={n_users}, backend={backend}...")
            results.append(benchmark_h3(data_path, n_users, backend, args.iterations))

    payload = {
        "metadata": {
            "benchmark": "preprocessing.latlng_to_h3",
            "dataset": "yjmob100k",
    ...
    output_path = args.output or (get_default_output_dir() / "fastmob_h3_yjmob_speed.json")
    write_json(payload, output_path)
    print(f"Wrote {output_path}")
    for r in results:
        print(
            f"  n_users={r['n_users']:>7} backend={r['backend']:<7} "
            f"rows={r['n_rows']:>10} min_seconds={r['minimum_seconds']:.4f} "
            f"rows/sec={r['rows_per_second']:.0f}"
        )
    return 0
