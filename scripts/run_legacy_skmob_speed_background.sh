#!/usr/bin/env bash
# Run original scikit-mobility speed baselines first, then regenerate comparison plots.
...
log "legacy skmob speed run started"
log "python: $PYTHON"
log "results: $RESULTS_DIR"
log "real_entropy is not run by the default individual speed suite"
...
run_always plot_speed_comparisons_with_legacy \
    bash scripts/plot_all_comparisons.sh --env-dir "$RESULTS_DIR" --input-order raw

touch "$RESULTS_DIR/legacy_skmob_speed.done"
log "legacy skmob speed run complete"
