...
# no prebuilt wheel past 3.10 -- see pyproject.toml's dev-pymove comment):
...
if [ ! -d ".venv-pymove" ]; then
    ...
    echo "  Set it up first: bash scripts/setup_env.sh --venv .venv-pymove --python 3.9 --pymove"
    exit 1
fi
