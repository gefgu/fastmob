#!/usr/bin/env bash
# Re-run skmob2 memory benchmarks with memray (1 iteration each) and regenerate plots.
...
# ── Phase 1: skmob2 memory benchmarks (memray, 1 iteration, raw order) ───────
...
echo ""
echo "=== Phase 1: skmob2 memory benchmarks (memray, 1 iteration) ==="
...
# plot_all_comparisons.sh falls back to standalone skmob2-only plots.
echo ""
