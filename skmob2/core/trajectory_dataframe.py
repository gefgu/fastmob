...
class TrajDataFrame(BaseDataFrame):
    ...
    def to_flowdataframe(self, tessellation, self_loops: bool = True):
        ...
        Notes
        -----
        Requires ``skmob2[data]``::

            pip install "skmob2[data]"

        Examples
        --------
        >>> import skmob2
        >>> tdf = skmob2.data.load_dataset("foursquare_nyc")  # doctest: +SKIP
        >>> from skmob2.tessellation.tilers import tiler  # doctest: +SKIP
    ...
    def to_geodataframe(self):
        ...
        try:
            ...
            raise ImportError('geopandas is required: pip install "skmob2[data]"') from exc
    ...
    def mapping(self, tessellation, remove_na: bool = False) -> "TrajDataFrame":
        ...
        try:
            ...
            raise ImportError('geopandas and shapely are required: pip install "skmob2[data]"') from exc
        ...
        if all(isinstance(x, Polygon) for x in tess.geometry):
            ...
            from skmob2.utils.utils import nearest
    ...
    def settings_from(self, other: "TrajDataFrame") -> None:
        ...
        ... })
        >>> tdf1 = skmob2.TrajDataFrame(df.copy())
        >>> tdf2 = skmob2.TrajDataFrame(df.copy(), parameters={"source": "gps"})
    ...
    # ------------------------------------------------------------------
    # Visualization methods  (require skmob2[visualization])
    # ------------------------------------------------------------------
    ...
    def plot_trajectory(
        ...
        style_function : callable, optional
            ...
            Defaults to ``skmob2.utils.plot.traj_style_function``.
        tiles : str, optional
        ...
        """
        try:
            from skmob2.utils import plot
        except ImportError as exc:
            raise ImportError(
                'Visualization requires extra dependencies: pip install "skmob2[visualization]"'
            ) from exc
    ...
    def plot_stops(
        ...
        (output of :func:`skmob2.preprocessing.stay_locations`).
    ...
    def plot_diary(
        ...
        ``skmob2.preprocessing.cluster``), with ``cluster`` and
        ...
        >>> tdf = skmob2.data.load_dataset("foursquare_nyc")  # doctest: +SKIP
        >>> stdf = tdf.stay_locations(minutes_for_a_stop=20)  # doctest: +SKIP
        >>> cstdf = skmob2.preprocessing.cluster(stdf)  # doctest: +SKIP
