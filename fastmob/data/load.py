...
def load_dataset(name, drop_columns=False, auth=None, show_progress=False):
    ...
    try:
        ...
        if exc.name and (exc.name.startswith("fastmob.data.datasets") or exc.name == short_name):
    ...
    full_path_files = _fastmob_downloader(
        dataset_info["url"],
        hash_value,
        auth=auth,
...
def _cache_dir() -> Path:
    ...
    if root:
        return Path(root) / "fastmob_data"
    return Path.home() / ".cache" / "fastmob_data"


def _fastmob_downloader(url, known_hash, download_format=None, auth=(), show_progress=False):
    del show_progress
