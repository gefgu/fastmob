...
def _wasserstein_distance():
    try:
        ...
        raise ImportError(
            "contact-network Wasserstein comparisons require scipy. Install it with `pip install fastmob[fitting]`."
        ) from exc
...
def co_presence_graph_from_visits(
    ...
    *,
    ...
    --------
    >>> import pandas as pd
    >>> from fastmob.measures.collective.contact_network import co_presence_graph_from_visits
    >>> visits = pd.DataFrame(
    ...     {
    ...         "uid": ["a", "b", "c", "a", "b"],
...
def infer_social_ties(
    ...
    *,
    ...
    --------
    >>> import numpy as np
    >>> from fastmob.measures.collective.contact_network import NetworkGraph, infer_social_ties
    >>> graph = NetworkGraph(
    ...     node_count=4,
...
co_presence_graph_from_visits.__module__ = "fastmob.measures.collective"
infer_social_ties.__module__ = "fastmob.measures.collective"
