...
class RecastTemporalGraph:
    ...
    def event(self, index: int) -> RecastEventGraph:
        ...
        return RecastEventGraph(self.node_count, self.edge_from.slice(start, end - start), self.edge_to.slice(start, end - start))
...
def _prepare(staypoints: Staypoints, locations: Locations) -> _PreparedStaypoints:
    ...
    work = nw.from_native(assigned.df, eager_only=True).select([uid_col, "location_id", start_col, end_col]).drop_nulls()
    if len(work) == 0:
        return _PreparedStaypoints(pa.array([], type=pa.null()), pa.array([], type=pa.uint32()), pa.array([], type=pa.uint32()), pa.array([], type=pa.int64()), pa.array([], type=pa.int64()))
    ...
    return _PreparedStaypoints(
        ...
        pc.cast(uid_codes, pa.uint32()), pc.cast(location_codes, pa.uint32()),
...
def _result(prepared: _PreparedStaypoints, values: tuple, *, min_minutes_for_encounter: int, p_rnd: float, random_replicates: int, seed: int) -> RecastResult:
    edge_from, edge_to, persistence, overlap, classes, p_threshold, o_threshold, time_steps = values
    edge_from, edge_to, persistence, overlap, classes = map(pa.array, (edge_from, edge_to, persistence, overlap, classes))
    return RecastResult(pc.take(prepared.node_ids, edge_from), pc.take(prepared.node_ids, edge_to), persistence, overlap, classes, float(p_threshold), float(o_threshold), p_rnd, random_replicates, seed, min_minutes_for_encounter, int(time_steps))


def recast_from_staypoints(staypoints: Staypoints, locations: Locations, *, min_minutes_for_encounter: int = 5, p_rnd: float = 1e-3, random_replicates: int = 5, seed: int = 42) -> RecastResult:
    ...
    values = recast_classify(prepared.node_count, prepared.users, prepared.locations, prepared.starts, prepared.ends, min_contact_ms, p_rnd, random_replicates, seed)
    return _result(prepared, values, min_minutes_for_encounter=min_minutes_for_encounter, p_rnd=p_rnd, random_replicates=random_replicates, seed=seed)


def temporal_graph_from_staypoints(staypoints: Staypoints, locations: Locations, *, min_minutes_for_encounter: int = 5) -> RecastTemporalGraph:
    ...
    prepared = _prepare(staypoints, locations)
    if prepared.node_count == 0:
        return RecastTemporalGraph(0, prepared.node_ids, pa.array([], type=pa.int64()), pa.array([0], type=pa.uint64()), pa.array([], type=pa.uint32()), pa.array([], type=pa.uint32()))
    from fastmob._core import recast_event_graphs
    windows, offsets, edge_from, edge_to = recast_event_graphs(prepared.users, prepared.locations, prepared.starts, prepared.ends, min_contact_ms)
    return RecastTemporalGraph(prepared.node_count, prepared.node_ids, pa.array(windows), pa.array(offsets), pa.array(edge_from), pa.array(edge_to))
...
def t_rnd(temporal_graph: RecastTemporalGraph, *, random_replicates: int = 5, seed: int = 42) -> list[RecastTemporalGraph]:
    ...
    return [RecastTemporalGraph(temporal_graph.node_count, temporal_graph.node_ids, *map(pa.array, recast_t_rnd(temporal_graph.node_count, temporal_graph.window_starts_ms, temporal_graph.edge_offsets, temporal_graph.edge_from, temporal_graph.edge_to, replica, seed))) for replica in range(random_replicates)]


def validate_recast_from_staypoints(staypoints: Staypoints, locations: Locations, *, min_minutes_for_encounter: int = 5, p_rnd: float = 1e-3, random_replicates: int = 5, seed: int = 42) -> RecastValidation:
    ...
    prepared = _prepare(staypoints, locations)
    if prepared.node_count == 0:
        result = recast_from_staypoints(staypoints, locations, min_minutes_for_encounter=min_minutes_for_encounter, p_rnd=p_rnd, random_replicates=random_replicates, seed=seed)
        graph = temporal_graph_from_staypoints(staypoints, locations, min_minutes_for_encounter=min_minutes_for_encounter)
        empty = pa.array([], type=pa.float64()); comparison = RecastClusteringComparison(graph.window_starts_ms, empty, empty, empty)
    ...
    values = recast_validate(prepared.node_count, prepared.users, prepared.locations, prepared.starts, prepared.ends, min_contact_ms, p_rnd, random_replicates, seed)
    result = _result(prepared, values[:8], min_minutes_for_encounter=min_minutes_for_encounter, p_rnd=p_rnd, random_replicates=random_replicates, seed=seed)
    graph = RecastTemporalGraph(prepared.node_count, prepared.node_ids, *map(pa.array, values[18:22]))
    full = RecastClusteringComparison(graph.window_starts_ms, pa.array(values[12]), pa.array(values[13]), pa.array(values[14]))
    random_only = RecastClusteringComparison(graph.window_starts_ms, pa.array(values[15]), pa.array(values[16]), pa.array(values[17]))
    return RecastValidation(result, graph, pa.array(values[8]), pa.array(values[9]), pa.array(values[10]), pa.array(values[11]), full, random_only)


for _item in (RecastClass, RecastEventGraph, RecastTemporalGraph, RecastResult, RecastClusteringComparison, RecastValidation, recast_from_staypoints, temporal_graph_from_staypoints, rnd, t_rnd, validate_recast_from_staypoints):
