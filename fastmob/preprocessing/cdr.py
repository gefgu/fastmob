...
def _timestamp_seconds(df: nw.DataFrame, column: str) -> list[float]:
    return (
        df.with_columns((nw.col(column).dt.timestamp("ms") / 1000.0).alias("__fastmob_ts_s__"))
        .get_column("__fastmob_ts_s__")
        .to_list()
    )
...
def cdr_to_visitation_df(
    ...
    --------
    >>> import pandas as pd
    >>> from fastmob.preprocessing import cdr_to_visitation_df
    >>> cdr = pd.DataFrame(
    ...     {
    ...         "user_id": ["u1", "u1", "u1", "u1"],
...
def cdr_to_trips_df(
    ...
    end_timestamps_s = (
        df.with_columns(
            ...
            .alias("__fastmob_end_ts_s__")
        )
        .get_column("__fastmob_end_ts_s__")
        .to_list()
    )
