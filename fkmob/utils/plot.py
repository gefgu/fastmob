"""Visualization utilities for fkmob TrajDataFrame and FlowDataFrame.
...
plots) or fkmob ``FlowDataFrame`` objects (for flow and tessellation plots).
"""
...
from fkmob.utils.utils import get_geom_centroid

# Column name constants (mirror fkmob core defaults)
_UID = "uid"
_LAT = "lat"
_LNG = "lng"
_DATETIME = "datetime"
...
def plot_diary(
    ...
    Requires a clustered stop DataFrame (output of ``fkmob.preprocessing.cluster``),
...
def plot_flows(
    ...
    ----------
    fdf : FlowDataFrame
        fkmob FlowDataFrame with a tessellation attached.
    map_f : folium.Map, optional
        Existing map. If ``None``, a new map is created.
