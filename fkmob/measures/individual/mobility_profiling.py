...
_TRAJECTORY_TIMESTAMP_COL = "__fkmob_trajectory_timestamp__"
...
def _apply_cold_start_strategy(
    ...
    if cold_start_strategy == "frequency":
        ...
        from fkmob._core import cluster_kmeans as _cluster_kmeans
...
def intermittance_and_degree_of_return(
    ...
    *explorations* (new places) and *returns* (revisits or home/work visits).
    ...
    --------
    >>> import pandas as pd
    >>> from fkmob.measures.individual import intermittance_and_degree_of_return
    >>> visits = pd.DataFrame(
    ...     {
...
def exploration_profiling(
    ...
    * **scouters** — low degree of return; mostly explore new places.
    ...
    ImportError
        If the compiled ``fkmob._core`` extension is not available (run
        ``maturin develop`` first).

    Examples
    --------
    >>> import pandas as pd
    >>> from fkmob.measures.individual import exploration_profiling
    >>> rows = []
    >>> patterns = {
    ...     "u1": ["a", "b", "c", "d", "a"],
    ...     "u2": ["a", "b", "a", "c", "a", "d"],
    ...
    """
    from fkmob._core import cluster_gmm, cluster_kmeans
