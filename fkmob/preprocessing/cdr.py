...
from fkmob._core import cdr_approx_travel_minutes as _cdr_approx_travel_minutes
from fkmob._core import cdr_trip_indices as _cdr_trip_indices
from fkmob._core import cdr_visitation_stays as _cdr_visitation_stays
...
def _timestamp_seconds(df: nw.DataFrame, column: str) -> list[float]:
    return (
        df.with_columns((nw.col(column).dt.timestamp("ms") / 1000.0).alias("__fkmob_ts_s__"))
        .get_column("__fkmob_ts_s__")
        .to_list()
    )
...
def cdr_to_visitation_df(
    ...
    --------
    >>> import pandas as pd
    >>> from fkmob.preprocessing import cdr_to_visitation_df
    >>> cdr = pd.DataFrame(
    ...     {
    ...         "user_id": ["u1", "u1", "u1", "u1"],
...
def cdr_to_trips_df(
    ...
    end_timestamps_s = (
        df.with_columns(
            ...
            .alias("__fkmob_end_ts_s__")
        )
        .get_column("__fkmob_end_ts_s__")
        .to_list()
    )
