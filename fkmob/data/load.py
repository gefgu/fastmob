...
from fkmob.data._frames import FlowDataFrame, TrajDataFrame
...
def load_dataset(name, drop_columns=False, auth=None, show_progress=False):
    ...
    Examples
    --------
    >>> from fkmob.data import load_dataset, list_datasets
    ...
    try:
        module = importlib.import_module(f".datasets.{short_name}.{short_name}", package="fkmob.data")
    except ModuleNotFoundError as exc:
        if exc.name and (exc.name.startswith("fkmob.data.datasets") or exc.name == short_name):
    ...
    full_path_files = _fkmob_downloader(
        dataset_info["url"],
        hash_value,
        auth=auth,
...
def list_datasets(details=False, data_types=None):
    ...
    Examples
    --------
    >>> from fkmob.data import list_datasets
    >>> list_datasets()
...
def get_dataset_info(name):
    ...
    Examples
    --------
    >>> from fkmob.data import get_dataset_info
...
def _cache_dir() -> Path:
    ...
    if root:
        return Path(root) / "fkmob_data"
    return Path.home() / ".cache" / "fkmob_data"


def _fkmob_downloader(url, known_hash, download_format=None, auth=(), show_progress=False):
    del show_progress
