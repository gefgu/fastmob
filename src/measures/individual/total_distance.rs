...
fn total_distance_indexed_impl(
    latitudes: &[f64],
    longitudes: &[f64],
    indices: &[usize],
    ranges: &[(usize, usize)],
...
#[pyfunction]
pub(crate) fn total_distance_batch_km(
    latitudes: Vec<f64>,
    longitudes: Vec<f64>,
    ranges: Vec<(usize, usize)>,
) -> PyResult<Vec<f64>> {
    total_distance_impl(&latitudes, &longitudes, &ranges)
}

#[pyfunction]
pub(crate) fn total_distance_numpy(
...
#[pyfunction]
pub(crate) fn total_distance_indexed_numpy<'py>(
    py: Python<'py>,
...
#[pyfunction]
pub(crate) fn total_distance_arrow(
    latitudes: PyArray,
    longitudes: PyArray,
    ranges: Vec<(usize, usize)>,
) -> PyResult<Vec<f64>> {
...
#[pyfunction]
pub(crate) fn total_distance_indexed_arrow(
    latitudes: PyArray,
    longitudes: PyArray,
    ...
    let longitudes = as_f64_array(longitudes, "longitudes")?;
    let ranges = ranges_from_starts_ends(starts.as_slice()?, ends.as_slice()?)?;
    Ok(f64_results_into_arrow(total_distance_indexed_impl(
