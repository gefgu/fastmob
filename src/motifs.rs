...
// ---------------------------------------------------------------------------
// canonical_adjacency_form — public PyO3 wrapper
// ---------------------------------------------------------------------------
...
pub(crate) fn compute_daily_motifs(
        ...
        || durations.len() != n
    {
        return Err(PyValueError::new_err(
            "All input column vectors must have the same length",
        ));
    }
    if user_id_labels.len() != user_ranges.len() {
        return Err(PyValueError::new_err(
            "user_id_labels and user_ranges must have the same length",
        ));
    }
