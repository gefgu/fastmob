    ...
    for &(start, end) in ranges {
        if start > end || end > n_tokens {
            return Err(PyValueError::new_err(
                "ranges must be valid half-open intervals into tokens",
            ));
        }
    }
    Ok(())
...
#[pyfunction]
pub(crate) fn trajectory_entropy_batch(
    py: Python<'_>,
    tokens: Vec<String>,
    ranges: Vec<(usize, usize)>,
    normalized: bool,
...
#[pyfunction]
pub(crate) fn trajectory_predictability_batch(
    py: Python<'_>,
    tokens: Vec<String>,
    ranges: Vec<(usize, usize)>,
    ...
    let rows: Vec<(f64, f64, usize, usize)> = py.detach(|| {
        ranges
            .par_iter()
            .map(|&(start, end)| {
                let sequence = &tokens[start..end];
